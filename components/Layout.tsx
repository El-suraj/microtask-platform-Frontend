      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-primary-800 bg-primary-950/30">
            {/* Demo Role Switcher */}
            <div className="mb-4">
              <label className="text-xs text-primary-300 font-semibold mb-1 block">DEMO: SWITCH ROLE</label>
              <div className="relative">
                <select 
                  value={user.role}
                  onChange={(e) => switchRole(e.target.value as UserRole)}
                  className="w-full appearance-none bg-primary-800 border border-primary-700 text-primary-100 text-xs rounded px-2 py-1.5 focus:ring-1 focus:ring-primary-400 outline-none cursor-pointer hover:bg-primary-700 transition-colors"
                >
                  <option value={UserRole.WORKER}>Worker View</option>
                  <option value={UserRole.EMPLOYER}>Employer View</option>
                  <option value={UserRole.ADMIN}>Admin View</option>
                </select>
                <ChevronDown size={14} className="absolute right-2 top-1/2 -translate-y-1/2 text-primary-400 pointer-events-none" />
              </div>
            </div>
