    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard 
          ...
          value={submissions.status === 'active' ? submissions.length : 10}
          icon={<CheckCircle size={20} />} 
        />
        <StatCard 
          title="Pending Review" 
          value={submissions.status === 'pending' ? submissions.length : 0} 
          icon={<Clock size={20} />} 
        />
        <StatCard 
          title="Avg. Rating" 
            ...
            {tasks.slice(0, 3).map((task) => (
              <div key={task.id} className="p-4 sm:p-6 flex items-start sm:items-center gap-4 hover:bg-slate-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">{task.category} • {task.timeEstimate || "Flexible"}</p>
                </div>
                <div className="text-right">
                  <span className="block font-bold text-slate-900">{formatCurrency(task.reward)}</span>
                  <Badge color="green">{task.spotsTaken}/{task.spotsTotal} Spots</Badge>
                </div>
              </div>
            ))}
          </div>
